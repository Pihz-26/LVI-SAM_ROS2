...
//         0,          1    ]
tf2::Transform Transform_imu_lidar;
#endif
...
void readParameters(rclcpp::Node::SharedPtr node)
{
    ...
    node->declare_parameter<std::vector<double>>(PROJECT_NAME + "/extrinsicTranslation", std::vector<double>());
    node->declare_parameter<std::vector<double>>(PROJECT_NAME + "/extrinsicRotation", std::vector<double>());
    node->get_parameter(PROJECT_NAME + "/extrinsicTranslation", t_imu_lidar_V);
    node->get_parameter(PROJECT_NAME + "/extrinsicRotation", R_imu_lidar_V);
    ...
    if (std::abs(R_tmp.determinant()) <= 0.9) // 防止配置文件中写错，这里加一个断言判断一下
    {
        RCLCPP_ERROR(node->get_logger(), "Invalid rotation matrix determinant in configuration file.");
        return;
    } 
    ...
    Transform_imu_lidar = tf2::Transform(tf2::Quaternion(Q_imu_lidar.x(), Q_imu_lidar.y(), Q_imu_lidar.z(), Q_imu_lidar.w()), 
        tf2::Vector3(t_imu_lidar(0), t_imu_lidar(1), t_imu_lidar(2)));

    RCLCPP_WARN_STREAM(node->get_logger(), "=vins-feature_tracker read R_imu_lidar : =====================");
    std::cout << R_imu_lidar << std::endl;
     RCLCPP_WARN_STREAM(node->get_logger(), "=vins-feature_tracker read t_lidar_imu : =====================");
