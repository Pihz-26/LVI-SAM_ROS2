...
public:
    ...
    std::atomic<bool> is_initialized_{!reLocalization};
    rclcpp::TimerBase::SharedPtr rl_init_timer_;
    pcl::PointCloud<PointType>::Ptr corner_points_map;
    pcl::PointCloud<PointType>::Ptr surf_points_map;
    pcl::PointCloud<PointType>::Ptr global_points_map;
    Eigen::Isometry3d T_map_init_;
    Eigen::Isometry3d T_map_odom_;
    rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pubFullPoints;  // 这个用来接收整合点云，然后在estimate的位姿下给展示出来
    float points_match_rate;
    ...
    mapOptimization(const rclcpp::NodeOptions & options) : ParamServer("lio_sam_mapOptimization", options)
    {
        ...
        pubFullPoints = this->create_publisher<sensor_msgs::msg::PointCloud2>("lio_sam/feature/cloud_full", 1);

        br = std::make_unique<tf2_ros::TransformBroadcaster>(this);
        RCLCPP_INFO(this->get_logger(), "是否开启地图定位模式： %d", reLocalization);
        if (!reLocalization) {initialSub();}
        else {reLocationInitial();}
        ...
        auto saveMapService = [this](const std::shared_ptr<rmw_request_id_t> request_header, const std::shared_ptr<lio_sam::srv::SaveMap::Request> req, std::shared_ptr<lio_sam::srv::SaveMap::Response> res) -> void {
            ...
            if(req->destination.empty()) saveMapDirectory = savePCDDirectory;  // td::getenv("HOME") + savePCDDirectory;
            else saveMapDirectory = req->destination; // std::getenv("HOME") + req->destination;
    ...
    void initialSub() {
        ...
        RCLCPP_INFO(this->get_logger(), "Finished SUB initial");
    }
    ...
    void reLocationInitial() {
        ...
        // 导入已有地图
        if (pcl::io::loadPCDFile<PointType>(surfMapPath, *surf_points_map) == -1) {
        ...
        map_filter.setInputCloud(surf_points_map);
        map_filter.filter(*surf_points_map);       // 降采样后点数≈0.5M~1M
        map_filter.setInputCloud(corner_points_map);
        ...
        rl_init_timer_ = this->create_wall_timer(
            std::chrono::seconds(1),
            std::bind(&mapOptimization::checkInitialTf, this)
        );
        std::cout << "Build Timer" << std::endl;
    ...
    void checkInitialTf() {
        ...
        try {
            geometry_msgs::msg::TransformStamped transform = tf_buffer_->lookupTransform(
                "map", "init",
                tf2::TimePointZero,  // 获取最新可用的 TF
                std::chrono::milliseconds(100)  // 超时 100ms
            );
            std::cout << "TF F" << std::endl;
            // 转换为Eigen格式存储
            T_map_init_.translation() = Eigen::Vector3d(
                transform.transform.translation.x,
                transform.transform.translation.y,
                transform.transform.translation.z);
            T_map_init_.linear() = Eigen::Matrix3d(
                Eigen::Quaterniond(
            ...
            // 打印调试信息
            Eigen::Vector3d rpy = T_map_init_.linear().eulerAngles(0, 1, 2);
            RCLCPP_INFO(get_logger(), "Initial pose captured - Position: [%.2f, %.2f, %.2f] RPY: [%.2f, %.2f, %.2f]",
                    T_map_init_.translation().x(), T_map_init_.translation().y(), T_map_init_.translation().z(),
                    rpy.x()*180/M_PI, rpy.y()*180/M_PI, rpy.z()*180/M_PI);


            is_initialized_ = true;   
            initialSub();
            rl_init_timer_->cancel();
        } catch (const tf2::TransformException& ex) {
             RCLCPP_WARN_THROTTLE(get_logger(), *this->get_clock(), 1000, 
            "Waiting for initial TF from [map] to [init]: %s", ex.what());
        }
    } 


    Eigen::Isometry3d arrayToIsometry(const float T[6]) {
        Eigen::Vector3d t(T[3], T[4], T[5]);
        Eigen::AngleAxisd rollAngle(T[0], Eigen::Vector3d::UnitX());
        Eigen::AngleAxisd pitchAngle(T[1], Eigen::Vector3d::UnitY());
        Eigen::AngleAxisd yawAngle(T[2], Eigen::Vector3d::UnitZ());
        Eigen::Quaterniond q = yawAngle * pitchAngle * rollAngle;
        Eigen::Isometry3d T_isometry = Eigen::Isometry3d::Identity();
        T_isometry.rotate(q);
        T_isometry.pretranslate(t);
        return T_isometry;
    }

    Eigen::Matrix4f arrayToMatrix4f(const float T[6]) 
    {
        // 1. 构造旋转：使用 float 精度，避免不必要的 double 转换
        //    注意：Eigen::AngleAxis 和 Eigen::Quaternion 默认使用 double 精度。
        //    为了保持 float 精度，我们将使用 Matrix4f 的构造方式或强制转换。

        // 使用 Eigen::Matrix3f 来构造旋转矩阵
        Eigen::AngleAxisf rollAngle(T[0], Eigen::Vector3f::UnitX());
        Eigen::AngleAxisf pitchAngle(T[1], Eigen::Vector3f::UnitY());
        Eigen::AngleAxisf yawAngle(T[2], Eigen::Vector3f::UnitZ());
        ...
        // 设置旋转部分 (左上角 3x3)
        T_matrix.block<3, 3>(0, 0) = q.toRotationMatrix();
    ...
    void showPointCloud() {
        ...
        cloudMsg.header.stamp = timeLaserInfoStamp; // 使用当前的雷达时间戳
        cloudMsg.header.frame_id = "map"; // 设定坐标系为 /map

        pubFullPoints->publish(cloudMsg);
    }
    ...
    void laserCloudInfoHandler(const lio_sam::msg::CloudInfo::SharedPtr msgIn)
    {
        ...
        *laserCloudFull += *laserCloudCornerLast;
        *laserCloudFull += *laserCloudSurfLast;
        ...
        if (timeLaserInfoCur - timeLastProcessing >= mappingProcessInterval)
        {
            ...
            // RCLCPP_INFO(this->get_logger(), "extractSurroundingKeyFrames Finished");

            downsampleCurrentScan();
            // RCLCPP_INFO(this->get_logger(), "downsampleCurrentScan Finished");

            scan2MapOptimization();
            // RCLCPP_INFO(this->get_logger(), "scan2MapOptimization Finished");

            saveKeyFramesAndFactor();
            // RCLCPP_INFO(this->get_logger(), "saveKeyFramesAndFactor Finished");

            correctPoses();
            // RCLCPP_INFO(this->get_logger(), "correctPoses Finished");

            publishOdometry();
            // RCLCPP_INFO(this->get_logger(), "publishOdometry Finished");

            publishFrames();
            // RCLCPP_INFO(this->get_logger(), "publishFrames Finished");
        }
    }
    ...
    void publishGlobalMap()
    {
        ...
        if (cloudKeyPoses3D->points.empty() == true)
            return;
        RCLCPP_INFO(this->get_logger(), "cloudKeyPoses3D size: %ld",cloudKeyPoses3D->size());
        ...
        mtx.lock();
        RCLCPP_INFO(this->get_logger(), " Start Search(612)");
        kdtreeGlobalMap->setInputCloud(cloudKeyPoses3D);
        kdtreeGlobalMap->radiusSearch(cloudKeyPoses3D->back(), globalMapVisualizationSearchRadius, pointSearchIndGlobalMap, pointSearchSqDisGlobalMap, 0);
        RCLCPP_INFO(this->get_logger(), " Finish Search(612)");
        mtx.unlock();
    ...
    {
        if (loopClosureEnableFlag == false || reLocalization == true)
            return;
    ...
    void performLoopClosure()
    {
        ...
        *copy_cloudKeyPoses6D = *cloudKeyPoses6D;
        ...
        // extract cloud
        RCLCPP_INFO(this->get_logger(), "Start extract cloud");
    ...
    bool detectLoopClosureDistance(int *latestID, int *closestID)
    {
        ...
        RCLCPP_INFO(this->get_logger(), " Start Search(768)");
        kdtreeHistoryKeyPoses->radiusSearch(copy_cloudKeyPoses3D->back(), historyKeyframeSearchRadius, pointSearchIndLoop, pointSearchSqDisLoop, 0);
        RCLCPP_INFO(this->get_logger(), " Finish Search(768)");
        ...
        // RCLCPP_INFO(this->get_logger(), " Finish (789)");
        if (loopKeyPre == -1 || loopKeyCur == loopKeyPre) {
            // RCLCPP_INFO(this->get_logger(), " Finish (791)");
            return false;
        }
        ...
        *closestID = loopKeyPre;
        // RCLCPP_INFO(this->get_logger(), " Finish (798)");
        return true;
    }
    ...
    void relevantPoseInitial() {
        // 本函数在 `updateInitialGuess` 中进行执行， 这个的目的是计算 map 和 odom之间的相对位姿，实际上 map->init 位姿已经存储在了 T_map_init_
        // 在 updateInitialGuess 执行中，初始化将整个读取的角点地图和平面地图全部给转移到odom的坐标系下，这样就可以保证lio-sam其中本身的点云一致性
        Eigen::Isometry3d T_odom_init = Eigen::Isometry3d::Identity();
        T_odom_init.rotate(Eigen::AngleAxisd(transformTobeMapped[2], Eigen::Vector3d::UnitZ()) *  // yaw
                      Eigen::AngleAxisd(transformTobeMapped[1], Eigen::Vector3d::UnitY()) *  // pitch
                      Eigen::AngleAxisd(transformTobeMapped[0], Eigen::Vector3d::UnitX())); // roll

        T_map_odom_ = T_map_init_ * T_odom_init.inverse();
    ...
    void updateInitialGuess()
    {
        ...
        if (cloudKeyPoses3D->points.empty())
        {
            ...
            if (reLocalization) relevantPoseInitial();
            return;
        }
    ...
    void extractNearby()
    {
        if (!reLocalization) {
            ...
            RCLCPP_INFO(this->get_logger(), " Start Search(1017)");
            kdtreeSurroundingKeyPoses->radiusSearch(cloudKeyPoses3D->back(), (double)surroundingKeyframeSearchRadius, pointSearchInd, pointSearchSqDis);
            RCLCPP_INFO(this->get_logger(), " Finish Search(1017)");
    ...
    void extractCloud() {
        ...
        currentPosition.x = transformTobeMapped[3];  // 当前x
        ...
        // RCLCPP_INFO(this->get_logger(), " Start Search(1105)");
        
        cornerKDTree->radiusSearch(currentPosition, pointSearchRadius, cornerPointIndices, cornerPointDistances);
        // RCLCPP_INFO(this->get_logger(), " Start Search(1105)");
        // 将搜索到的角点加入局部地图
        ...
        // RCLCPP_INFO(this->get_logger(), " Start Search(1114)");
        surfKDTree->radiusSearch(currentPosition, pointSearchRadius, surfPointIndices, surfPointDistances);
        // RCLCPP_INFO(this->get_logger(), " Start Search(1116)");
        // 将搜索到的平面点加入局部地图
        ...
        RCLCPP_INFO(
            this->get_logger(),
            "\033[1;34mExtracted points: Corner=%zu, Surf=%zu\033[0m",
    ...
    void extractSurroundingKeyFrames()
    {
        ...
        RCLCPP_INFO(this->get_logger(), "extractNearby 耗时： %f", duration);
    ...
    void cornerOptimization()
    {
        ...
        for (int i = 0; i < laserCloudCornerLastDSNum; i++)
        {
            ...
            if (pointSearchSqDis[4] < maxMatchDistance) {
    ...
    {   
        float total_residual_blocks = 0.;
        float total_points_num = laserCloudCornerLastDSNum + laserCloudSurfLastDSNum;
        ...
        for (int i = 0; i < laserCloudSurfLastDSNum; ++i){
            if (laserCloudOriSurfFlag[i] == true){
                ...
                total_residual_blocks++;  // 平面点残差块计数
            }
        }
        // reset flag for next iteration
        std::fill(laserCloudOriCornerFlag.begin(), laserCloudOriCornerFlag.end(), false);
        std::fill(laserCloudOriSurfFlag.begin(), laserCloudOriSurfFlag.end(), false);
        points_match_rate = total_residual_blocks/total_points_num;
        if (points_match_rate < matchRateThreshold+0.05){
            RCLCPP_INFO(this->get_logger(), "【匹配率为： %f】", points_match_rate);
        }
    ...
    bool LMOptimization(int iterCount)
    {
        ...
        if (laserCloudSelNum < 50) {
            RCLCPP_INFO(this->get_logger(), "Useful Points number is %d", laserCloudSelNum);
            return false;
        }
        // else {
        //     RCLCPP_INFO(this->get_logger(), "Useful Points number is %d", laserCloudSelNum);
    ...
    void scan2MapOptimization()
    {
        ...
        if (laserCloudCornerLastDSNum > edgeFeatureMinValidNum && laserCloudSurfLastDSNum > surfFeatureMinValidNum)
        {
            ...
            for (int iterCount = 0; iterCount < 30; iterCount++)
            {
                ...
                // if (reLocalization && (points_match_rate < matchRateThreshold)) {
                //     // 当进行重定位，并且特征点匹配率过小时，使用ICP来进行配准，重点是矫正位姿
                //     icp_map.setInputSource(laserCloudFull);
                //     pcl::PointCloud<PointType> aligned_cloud;
                //     Eigen::Matrix4f init_guess = arrayToMatrix4f(transformTobeMapped);
                //     icp_map.align(aligned_cloud, init_guess);
                //     Eigen::Matrix4f icp_pose = icp_map.getFinalTransformation();
                //     RCLCPP_INFO(this->get_logger(), "ICP 配准误差为： %f", icp_map.getFitnessScore());
                //     transformTobeMapped[3] = icp_pose(0, 3); // x
                //     transformTobeMapped[4] = icp_pose(1, 3); // y
                //     transformTobeMapped[5] = icp_pose(2, 3); // z
                //     Eigen::Matrix3f rotation_matrix = icp_pose.block<3, 3>(0, 0);
                //     Eigen::Vector3f euler_angles = rotation_matrix.eulerAngles(2, 1, 0);
                //     transformTobeMapped[0] = euler_angles[2]; // roll
                //     transformTobeMapped[1] = euler_angles[1]; // pitch
                //     transformTobeMapped[2] = euler_angles[0]; // yaw
                //     if (icp_map.getFitnessScore() < 0.01){
                //         break ;
                //     } else {
                //         continue;
                //     }
                ...
                {
                    RCLCPP_INFO(this->get_logger(), "LMOptimization Successed in iter %d", iterCount);
                    break;     
                }   
                else {
                    if (iterCount == 99) {
                        RCLCPP_INFO(this->get_logger(), "LMOptimization Failed");
                    }
                }      
            }
    ...
    {
        if (reLocalization) {
            Eigen::Isometry3d T_odom_pose_ = Eigen::Isometry3d::Identity();
            T_odom_pose_.translation() << pose_in.x, pose_in.y, pose_in.z;
            Eigen::Matrix3d R = (   Eigen::AngleAxisd(pose_in.yaw, Eigen::Vector3d::UnitZ()) *
                                    Eigen::AngleAxisd(pose_in.pitch, Eigen::Vector3d::UnitY()) *
                                    Eigen::AngleAxisd(pose_in.roll, Eigen::Vector3d::UnitX()))
                                    .toRotationMatrix();
            T_odom_pose_.linear() = R;
            Eigen::Isometry3d T_map_pose_ = T_map_odom_.cast<double>() * T_odom_pose_;


            geometry_msgs::msg::PoseStamped pose_stamped;
            pose_stamped.header.stamp = rclcpp::Time(pose_in.time * 1e9);
            pose_stamped.header.frame_id = odometryFrame;
            pose_stamped.pose.position.x = T_map_pose_.translation().x();
            pose_stamped.pose.position.y = T_map_pose_.translation().y();
            pose_stamped.pose.position.z = T_map_pose_.translation().z();
            tf2::Quaternion q;
            Eigen::Matrix3d R_p = T_map_pose_.rotation();
            Eigen::Vector3d euler = R_p.eulerAngles(2, 1, 0);
            q.setRPY(euler[2], euler[1], euler[0]);
    ...
    void publishOdometry()
    {
        ...
        if (lastIncreOdomPubFlag == false)
        {
            ...
            laserOdomIncremental.child_frame_id = "basae_link";
    ...
    void publishFrames()
    {
        ...
        if (pubPath->get_subscription_count() != 0)
        {
            ...
            if (reLocalization) {
                ...
                globalPath.header.frame_id = "map";         // 为了方便调试，此处先给map
            }
