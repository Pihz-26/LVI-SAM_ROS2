...
mapOptimization::mapOptimization(std::string node_name): ParamServer(node_name)
{
...
void mapOptimization::laserCloudInfoHandler(const lvi_sam::msg::CloudInfo::SharedPtr msgIn)
{
    // extract time stamp
...
gtsam::Pose3 mapOptimization::affine3fTogtsamPose3(const Eigen::Affine3f& thisPose)
{
...
void mapOptimization::loopHandler(const std_msgs::msg::Float64MultiArray::SharedPtr loopMsg)
{
...
void mapOptimization::performLoopClosure(const std_msgs::msg::Float64MultiArray::SharedPtr loopMsg)
{
...
void mapOptimization::loopFindNearKeyframes(const pcl::PointCloud<PointTypePose>::Ptr& copy_cloudKeyPoses6D,
...
void mapOptimization::loopFindKey(const std_msgs::msg::Float64MultiArray::SharedPtr loopMsg, 
...
void mapOptimization::performLoopClosureDetection()
{
    ...
    auto match_msg = std::make_shared<std_msgs::msg::Float64MultiArray>();
...
void mapOptimization::publishOdometry()
{
    ...
    tf2::Transform t_odom_to_lidar(q_odom_to_lidar, tf2::Vector3(transformTobeMapped[3], transformTobeMapped[4], transformTobeMapped[5])); 

    geometry_msgs::msg::TransformStamped transformStamped;
    transformStamped.header.stamp = timeLaserInfoStamp;  // 设置时间戳
    transformStamped.header.frame_id = "odom";  // 父框架
    transformStamped.child_frame_id = "lidar_link";  // 子框架
    // 设置平移和旋转
    transformStamped.transform.translation.x = t_odom_to_lidar.getOrigin().x();
    transformStamped.transform.translation.y = t_odom_to_lidar.getOrigin().y();
    transformStamped.transform.translation.z = t_odom_to_lidar.getOrigin().z();
    transformStamped.transform.rotation.x = t_odom_to_lidar.getRotation().x();
    transformStamped.transform.rotation.y = t_odom_to_lidar.getRotation().y();
    transformStamped.transform.rotation.z = t_odom_to_lidar.getRotation().z();
    transformStamped.transform.rotation.w = t_odom_to_lidar.getRotation().w();
...
int main(int argc, char** argv)
{
    ...
    auto node = std::make_shared<mapOptimization>("mapOptimization");
