...
IMUPreintegration::IMUPreintegration(std::string node_name): ParamServer(node_name)
{   
    // 接收 IMU数据 和 里程计数据
...
void IMUPreintegration::imuHandler(const sensor_msgs::msg::Imu::SharedPtr imuMsg)
{
    sensor_msgs::msg::Imu thisImu = imuConverter(*imuMsg);
    // publish static tf
    geometry_msgs::msg::TransformStamped map_to_odom_msg;
    map_to_odom_msg.header.stamp = thisImu.header.stamp;
    ...
    if (imuTime - last_path_time > 0.1)
    {
        ...
        while(!imuPath.poses.empty() && abs(imuPath.poses.front().header.stamp.sec - imuPath.poses.back().header.stamp.sec) > 3.0)
...
int main(int argc, char** argv)
{
    ...
    auto node = std::make_shared<IMUPreintegration>("IMUPreintegration");
