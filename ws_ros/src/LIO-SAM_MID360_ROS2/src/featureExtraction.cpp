...
public:
    ...
    rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pubMergedPoints;
        ...
        ParamServer("lio_sam_featureExtraction", options)
    {
        ...
        pubMergedPoints = create_publisher<sensor_msgs::msg::PointCloud2>(
            "lio_sam/feature/cloud_merged", 1);
    ...
    void publishFeatureCloud()
    {
        ...
        freeCloudInfoMemory();
        pcl::PointCloud<PointType>::Ptr mergedFeatures(new pcl::PointCloud<PointType>());
        *mergedFeatures += *cornerCloud;   // 添加角点
        *mergedFeatures += *surfaceCloud;  // 添加平面点
        // 发布合并后的点云
        publishCloud(pubMergedPoints, mergedFeatures, cloudHeader.stamp, lidarFrame);
